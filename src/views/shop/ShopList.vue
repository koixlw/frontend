    ...
    <div v-if="showDeleteConfirm" class="delete-confirm-overlay" @click="cancelDelete">
      <div class="delete-confirm-dialog" @click.stop>
        <div class="delete-confirm-content">
          <i class="icon-warning delete-confirm-icon"></i>
          <h4 class="delete-confirm-title">确认删除</h4>
          <p class="delete-confirm-message">确定要从购物车删除 "{{ deleteItemName }}" 吗？</p>
        </div>
        <div class="delete-confirm-actions">
          <button class="cancel-delete-btn" @click="cancelDelete">取消</button>
          <button class="confirm-delete-btn" @click="executeDelete">确定删除</button>
        </div>
      </div>
    </div>
...
const addToCart = (product) => {
  ...
  // 显示添加成功提示
  showToast(`${product.title} 已成功添加到购物车！`);
};
...
const confirmRemove = (productId, productName) => {
  deleteItemId.value = productId;
  deleteItemName.value = productName;
...
const executeDelete = () => {
  if (deleteItemId.value) {
    const itemName = deleteItemName.value || '商品';
    cartItems.value = cartItems.value.filter(item => item.id !== deleteItemId.value);
    // 保存到 localStorage
    localStorage.setItem('cartItems', JSON.stringify(cartItems.value));
    // 显示删除提示
    showToast(`${itemName} 已从购物车移除`);
  }
  cancelDelete();
};

// 从购物车移除商品（保留此函数，但不再直接调用）
